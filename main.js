document.addEventListener('DOMContentLoaded', () => {
    ...
    function typeEffect() {
        ...
        if (typingText) typingText.innerHTML = currentText;
    ...
    function validateField(input, error, minLength, regex = null, emptyMsg, invalidMsg) {
        ...
        if (value === '') {
            ...
            if (error) error.textContent = emptyMsg;
            return false;
        } else if (value.length < minLength) {
            input.classList.add('error');
            if (error) error.textContent = invalidMsg;
            return false;
    ...
    function checkMessageContent() {
        ...
        if (hasBadWord) {
            if (messageError) messageError.textContent = 'This message contains inappropriate words.';
    ...
    if (nameInput) nameInput.addEventListener('input', () => { if (nameInput.classList.contains('error')) validateName(); });
    if (emailInput) emailInput.addEventListener('input', () => { if (emailInput.classList.contains('error')) validateEmail(); });
    if (subjectInput) subjectInput.addEventListener('input', () => { if (subjectInput.classList.contains('error')) validateSubject(); });
    if (messageInput) messageInput.addEventListener('input', () => { if (messageInput.classList.contains('error')) validateMessage(); });
